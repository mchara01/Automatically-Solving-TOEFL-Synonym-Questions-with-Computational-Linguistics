...
*
* Compilation command: javac -classpath .:stdlib.jar TOEFL.java
* Execution command: java -classpath .:stdlib.jar TOEFL
*
* In this class we accomplished to build an intelligent system that can learn
* to answer questions like the ones in TOEFL exams. In order to do that, the system 
* will approximate the semantic similarity of any pair of words. The semantic
* similarity between two words is the measure of the closeness of their meanings.We
* measured the semantic similarity of pairs of words by first computing a semantic descriptor
* vector of each of the words, and then took the similarity measure as the cosine similarity
* between the two vectors. All methods that are needed for calculations are implemented below.
*/
...
public class TOEFL {
	/**
	 * This function uses the StringTokenizer to first split the sentences each time
	 * the correct character appears and the split the sentence in words each time
	 * the correct character appears. The words are put in an arrayList of Strings
	 * and those arrayLists are put into the arrayList of the arrayLists
	 * 
	 * @param text
	 *            The text we are going to tokenize.
	...
	/**
	 * We take the arrayList of file names and run through the files. For each file
	 * in the try-catch block we take each newL and append it to the stringbuilder
	 * and then use the first function to put the string as an element of the slff
	 * arrayList.
	 * 
	 * @param filenames
	 *            an arrayList which contains the names of the filenames we are
	 *            going to tokenize to arrayLists
	 * @return
	 **/
	...
	/**
	 * We visualize the arrayList of arrayLists as the text of sentences. Each
	 * arrayList of Strings in the bigger arrayList(text) is the sentence. In the
	 * sentence we have words(Strings) In the first for-each loop we make an
	...
	/**
	 * This method uses the cosine_similarity method in order to calculate which is
	 * the most similar word We create an array equal to the size of the array of
	 * choices we are given and check if the word is in our "dictionary"
	 * (semantic_descriptors). If not we give the value -1 making it completely
	 * opposite meaning, if it exists then we calculate it's similarity and put the
	 * value in the table We make check through the table and return the first word
	...
	/**
	 * This function uses a try catch block using the buffer to save the result in
	 * text file.
	 * 
	 * @param onoma
	 *            The name of the text file that will be created
	 * @param table
	 *            The name of the string that is going to be placed in the text file
	 */
	public static void Save(String onoma, String table) {
		try {
			File file = new File("out-" + onoma);
